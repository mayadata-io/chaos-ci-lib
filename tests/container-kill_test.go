...
var _ = Describe("BDD of container-kill experiment", func() {
	...
	Context("Check for Litmus components", func() {
		...
		It("Should check for creation of runner pod", func() {
			...
			var err error
			var experimentName = "container-kill"
			var engineName = "engine2"
			//Installing RBAC for the experiment
			err = pkg.InstallRbac(chaosTypes.ContainerKillRbacPath, pkg.GetEnv("APP_NS", "default"), experimentName, chaosTypes.Client)
			Expect(err).To(BeNil(), "Fail to create RBAC")
			klog.Info("Rbac has been created successfully !!!")

			//Creating Chaos-Experiment
			By("Creating Experiment")
			err = pkg.DownloadFile(experimentName+".yaml", chaosTypes.ContainerKillExperimentPath)
			...
			err = pkg.DownloadFile(experimentName+"-ce.yaml", chaosTypes.ContainerKillEnginePath)
			...
			Expect(err).To(BeNil(), "Failed to update the chaos interval")
			err = pkg.EditKeyValue(experimentName+"-ce.yaml", "TOTAL_CHAOS_DURATION", "value: '20'", "value: '"+pkg.GetEnv("TOTAL_CHAOS_DURATION", "20")+"'")
