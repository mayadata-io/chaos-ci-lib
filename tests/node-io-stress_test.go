...
var _ = Describe("BDD of node-io-stress experiment", func() {
	...
	Context("Check for Litmus components", func() {
		...
		It("Should check for creation of runner pod", func() {
			...
			var err error
			var experimentName = "node-io-stress"
			...
			err = pkg.InstallRbac(chaosTypes.NodeIOStressRbacPath, pkg.GetEnv("APP_NS", "default"), experimentName, client)
			...
			err = pkg.EditKeyValue(experimentName+"-ce.yaml", "TOTAL_CHAOS_DURATION", "value: '120'", "value: '"+pkg.GetEnv("TOTAL_CHAOS_DURATION", "60")+"'")
			Expect(err).To(BeNil(), "Failed to update total chaos duration")
			err = pkg.EditKeyValue(experimentName+"-ce.yaml", "FILESYSTEM_UTILIZATION_PERCENTAGE", "value: '10'", "value: '"+pkg.GetEnv("FILESYSTEM_UTILIZATION_PERCENTAGE", "10")+"'")
			Expect(err).To(BeNil(), "Failed to filesystem utilization percentage")
